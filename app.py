...
Fix 1  : Resolved Git merge conflict (removed <<<< ==== >>>> markers)
Fix 2  : Render port binding — listens on $PORT env var (set in CMD, not ENTRYPOINT)
"""
...
# ══════════════════════════════════════════════════════════════════════
#  GLOBAL CSS  — @import only (never <link> in Streamlit)
# ══════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
:root {
  ...
  --purple-bdr : rgba(168,85,247,0.25);
  --blue       : #3B82F6;
  --glow-g     : 0 0 24px rgba(34,197,94,0.18);
  --glow-p     : 0 0 24px rgba(168,85,247,0.18);
...
.block-container { padding: 2.5rem 3.5rem 6rem !important; max-width: 1080px !important; }
...
[data-testid="stFileUploader"] {
  ...
  transition: all .25s ease !important; box-shadow: var(--card-shadow) !important;
}
...
.stButton > button[kind="primary"] {
  ...
  border: none !important; border-radius: 10px !important; padding: .85rem 2.5rem !important;
...
.stDownloadButton > button {
  ...
  padding: .85rem 2rem !important; transition: all .2s ease !important;
}
...
.streamlit-expanderHeader {
  ...
  padding: .85rem 1.2rem !important; transition: border-color .18s, box-shadow .18s !important;
}
.streamlit-expanderHeader:hover {
  border-color: var(--border2) !important; box-shadow: var(--card-shadow) !important;
}
...
.stTextArea textarea {
  ...
  font-family: 'Inter',sans-serif !important; font-size: .85rem !important; line-height: 1.75 !important;
}
...
[data-testid="metric-container"]:hover {
  transform: translateY(-4px) !important; border-color: var(--green) !important;
  box-shadow: var(--card-shadow), var(--glow-g) !important;
}
...
[data-testid="metric-container"] [data-testid="stMetricValue"] {
  font-family: 'Inter',sans-serif !important; font-size: 2.4rem !important;
  font-weight: 800 !important; color: var(--text) !important; line-height: 1.1 !important;
}
...
def extract_image(b, name):
    try:
        ...
        return "", "Tesseract not installed. Run: apt-get install tesseract-ocr"
...
# ══════════════════════════════════════════════════════════════════════
#  UI HELPERS  — 100% inline styles, no class deps
# ══════════════════════════════════════════════════════════════════════
def tag(text, color="green"):
    c = {
        ...
        "gray"  : ("rgba(255,255,255,.06)","rgba(255,255,255,.12)","#888"),
    }
    bg, border, fg = c.get(color, c["gray"])
    return (f'<span style="font-family:Inter,sans-serif;font-size:.72rem;font-weight:600;'
            f'letter-spacing:.06em;text-transform:uppercase;background:{bg};color:{fg};'
            f'border:1px solid {border};padding:.25rem .75rem;border-radius:20px;white-space:nowrap;">{text}</span>')
...
<div style="padding:4rem 0 2rem;max-width:780px;">
  ...
  <h1 style="font-family:Inter,sans-serif;font-size:clamp(2.6rem,5.5vw,4rem);
       font-weight:900;letter-spacing:-.04em;line-height:1.08;color:#F0F0F0;margin:0 0 1.2rem;">
    Turn any meeting<br>
...
<p style="font-family:Inter,sans-serif;font-size:.68rem;font-weight:700;
   ...
   Accepted Formats
        ...
        for icon, txt in [("🎵","Audio — mp3, wav, m4a"), ("📄","PDF"),
...
<div style="display:flex;align-items:center;gap:10px;margin-bottom:.5rem;padding:.4rem .6rem;
  ...
  <span style="font-family:Inter,sans-serif;font-size:.82rem;color:#888;">{txt}</span>
</div>""", unsafe_allow_html=True)

        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("""
<p style="font-family:Inter,sans-serif;font-size:.8rem;color:#444;text-align:center;margin:0;">
  Built by <span style="color:#22C55E;font-weight:600;">Rishi Raj</span>
</p>""", unsafe_allow_html=True)
    return cm
...
def render_results(results, out):
    ...
    st.markdown(f"""
<div style="background:#111111;border:1px solid rgba(255,255,255,.12);border-radius:16px;
     padding:2.4rem 2.6rem;position:relative;overflow:hidden;
     box-shadow:0 1px 0 rgba(255,255,255,.04) inset,0 20px 60px rgba(0,0,0,.55),0 6px 20px rgba(0,0,0,.3);
     margin-bottom:1rem;">
...
<div style="display:flex;align-items:flex-start;gap:1.2rem;
     background:#111111;border:1px solid rgba(255,255,255,.07);border-radius:14px;
     padding:1.4rem 1.6rem;margin-bottom:.65rem;position:relative;overflow:hidden;
     box-shadow:0 1px 0 rgba(255,255,255,.04) inset,0 20px 60px rgba(0,0,0,.55),0 6px 20px rgba(0,0,0,.3);
     transition:border-color .2s,transform .2s;"
     onmouseover="this.style.borderColor='rgba(255,255,255,.16)';this.style.transform='translateY(-2px)'"
  ...
  <div style="min-width:2rem;height:2rem;border-radius:8px;flex-shrink:0;margin-left:.2rem;
       background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);
       display:flex;align-items:center;justify-content:center;">
    <span style="font-family:Inter,sans-serif;font-size:.8rem;font-weight:800;color:#22C55E;">{i}</span>
  </div>
...
def render_empty_state():
    st.markdown("""
<div style="background:#111111;border:1px solid rgba(255,255,255,.12);border-radius:20px;
     padding:5rem 2rem;text-align:center;margin-top:1rem;position:relative;overflow:hidden;
     box-shadow:0 1px 0 rgba(255,255,255,.04) inset,0 20px 60px rgba(0,0,0,.55),0 6px 20px rgba(0,0,0,.3);">
  ...
  <p style="font-family:Inter,sans-serif;font-size:1.5rem;font-weight:800;
     color:#F0F0F0;margin:0 0 .75rem;letter-spacing:-.03em;">Drop your meeting files</p>
